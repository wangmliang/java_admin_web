...
var findStaffExtendProperties = basePath + '/staff/findStaffExtendProperties.ajax';
var staffExtendProperties = [];
var add_validator;
var edit_validator;
jQuery(function($) {
	$.permCheck.run();
	...
	initStaffExtendProperties();
	add_validator = $('#addForm').validate( {
		rules : {
			'loginName' : {
				required : true,
		...
		submitHandler : function(form) {
			$.ajaxSubmit(create_staff_url, $(form).serializeArray(), function(
					data) {
				if (data.success) {
	...
	for (var i = 0 ; i < staffExtendProperties.length; i++){
		var checkRule = $.parseJSON(staffExtendProperties[i]['check']) || {};
		editRule = $.extend(true,editRule,checkRule);
	}
	edit_validator = $('#editForm').validate(editRule);
});
function initStaffExtendProperties(){
	 $.ajax({
		url : findStaffExtendProperties,
		async: false,
		success : function(data) {
			if (data.success == true) {
				staffExtendProperties = data.data;
				var html = template("staff-extend-properties-tmpl", data);
				$(html).insertAfter("#editForm-last");
			} else {
				Q_Alert_Fail(data.message);
			}
