    ...
    $("#customer-select").on("change", function() {
        if (isWYAdmin) {
            ...
            if ($("input[type=radio]:checked").val() == 3) {
                ...
                // 动态判断组合流量包Table是否展示
                $.ajaxSubmit(ctxPaths + '/customerInfo/get.ajax', {
                    'customerId' : $this.val()
                }, function(rtn) {
                    if (rtn.success) {
                        // 渠道直充合作伙伴
                        if (rtn.data.partnerInfo.partnerType == 2) {
                            $(".combo-package").hide();
                            $(".comboRadio").hide();
    ...
    if (pCustomerId) {
        $.ajaxSubmit(ctxPaths + '/customerInfo/get.ajax', {
            ...
            if (rtn.success) {
                var obj = {};
                obj["id"] = pCustomerId;
                obj["text"] = rtn.data.customerName;
...
function initProductOrderTable(dataList, flowType) {
    ...
    if (null != dataList && dataList.length > 0) {
        $.each(dataList, function(index, rowData) {
            ...
            if ($("input[type=radio]:checked").val() == 3) {
                html += "<td><input attrid='" + rowData.productId + "' name='flowProductInfoList[" + productIndex + "].productCount' class='numberFormat prodCountInput' value='"+ rowData.productCount +"' /></td>"
            } else {
            ...
            // 结算价格
            html += "<td><input attrid='" + rowData.productId + "' name='flowProductInfoList["+ productIndex +"].settlementAmount' class='numberFormat settlementAmountInput' value='" + rowData.settlementAmount + "' /></td>";
            // 销售价格
            if ($("input[type=radio]:checked").val() == 3) { // 流量加
                ...
                html += "<td><input attrid='" + rowData.productId + "' name='flowProductInfoList["+ productIndex +"].settlementPrice' class='numberFormat productPriceInput' value='" + rowData.settlementPrice + "'/></td>";
            }
...
function chooseProductBtnClickHandler() {
    $("#choose-product").on('click', function(e) {
        ...
        if (isMarketing || isWYAdmin) {
            ...
            url = ctxPaths + '/flowProductInfo/flowProductInfo.ajax?productType=1&customerId=' + customerId;
        }
        $.ajax({
            url : url,
            dataType : 'json',
        ...
        $.ajax({
            ...
            success : function(data, status) {
                if (data.success == true) {
                    ...
                    var baseFlowPackage = [], comboFlowPackage = [];
                    $.each(prodOrderList, function(n, value) {
                        // 基础包
...
function checkInputValue() {
    ...
    $.each($(".settlementAmountInput"), function(n, obj) {
        var value = $(obj).val();
        if (value == null || value == "") {
            Q_Alert_Fail("请输入结算价格");
            successFlag = false;
            return false;
        }
        if (value == 0) {
            Q_Alert_Fail("结算价格不能为0");
            successFlag = false;
            return false;
        }
    ...
    $.each($(".productPriceInput"), function(n, obj) {
        var value = $(obj).val();
        if (value == null || value == "") {
            Q_Alert_Fail("请输入销售价格");
            successFlag = false;
            return false;
        }
        if (value == 0) {
            Q_Alert_Fail("销售价格不能为0");
            successFlag = false;
            return false;
        }
        var doubleReg = /^\d{1,8}\.?\d{0,2}$/;
